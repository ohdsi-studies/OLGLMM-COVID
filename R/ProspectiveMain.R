...
#'                                         }  
#' @param dates     A named list containing the start and end dates for each wave investigated  
#'
#' @examples
#' \dontrun{
...
executeProspective <- function(
  ...
  if(run){
    ...
    jsonFileLocation <- lapply(
      names(dates), 
      function(x){
        file.path(outputFolder,x, 'control.json')
      }
      )
    names(jsonFileLocation) <- names(dates)
